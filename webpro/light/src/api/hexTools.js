export default class HexTools {
	...
	static num2Hex0x(num) {
		return "0x"+this.num2Hex(num).toUpperCase();
	}
	
	static bleBuffer(cmd,d1,d2) {
		let c = parseInt(cmd,16);
		let d1_16_str = this.num2Hex0x(d1);
		let d2_16_str = this.num2Hex0x(d2);
		let sum = c + d1 + d2;
		let ay = new Uint8Array([this.num2Hex0x(sum)]);
		// console.log(sum,ay[0]);
		let xor = ay[0] ^ parseInt("0xFF",16);
		let array = new Uint8Array(["0xA6",cmd,d1_16_str,d2_16_str,this.num2Hex0x(xor)]);
		return array;
	}
}