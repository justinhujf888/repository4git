...
<!--                <span class="text-sm text-red-600">可拖拽排序</span>-->
        ...
        <DataView :value="selMasterCompetition.competitionList" :pt="{
            ...
            <template #list="slotProps">
                <div class="col">
                    <div v-for="(element,index) in slotProps.items" :key="index" class="m-2 p-2 !relative !border-2 !border-gray-300 !border-solid between">
                        <IftaLabel>
...
function addItem() {
    ...
    competition.appId = selMasterCompetition.value.appId;
    competition.temp = 0;
...
function save() {
    lodash.forEach(selMasterCompetition.value.competitionList,(v)=>{
        let mid = selMasterCompetition.value.id;
        v.masterCompetition = Beans.masterCompetition();
        v.masterCompetition.id = mid;
    });
    workRest.updateCompetitionList({competitionList:selMasterCompetition.value.competitionList},(res)=>{
        if (res.status=="OK") {
