<template>
    <div class="grid grid-cols-12 gap-4 justify-center relative" :class="rootClass">
        ...
        <!--            </div>-->
        <div class="col-span-12 sm:col-span-6 md:col-span-4 lg:col-span-4 xl:col-span-4 p-0 lg:pb-8 mt-6 lg:mt-0" v-for="(jiang) of jiangDatas" v-animateonscroll="{ enterClass: 'animate-enter fade-in-10 animate-duration-1000 zoom-in-50' }">
            <div style="height: 160px; padding: 2px; border-radius: 10px;">
                <div class="p-4 h-full center col gap-4" style="border-radius: 8px">
                    ...
                    <!--                        </div>-->
                    <jbText class="text-4xl siyuansongheavy" :colors="buildColors(jiang.name.colors)" :fromto="jiang.name.des+'deg'">{{jiang.name.text}}</jbText>
                    <div class="leading-8 col center" :class="jiangTextClass">
                        <span class="font-semibold text-xl _mix-blend-difference">{{jiang.jiangCount}}</span>
                        <span class="text-base _mix-blend-difference">{{jiang.jiangJing}}</span>
                        <span class="text-base _mix-blend-difference">{{jiang.jiangPing}}</span>
                    </div>
                </div>
            </div>
        </div>
...
const props = defineProps({
    ...
    jiangTextClass: {type:String,default:""}
});
const jiangDatas = defineModel("jiangDatas");
