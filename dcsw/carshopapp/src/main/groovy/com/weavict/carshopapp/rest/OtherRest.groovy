...
//                productsBean.uploadProductImages(img);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Path("/content2Oss")
    String content2Oss(@RequestBody Map<String,Object> query)
    {
        try
        {
            //oss
            OSSClient ossClient = OtherUtils.genOSSClient();
            ossClient.putObject(OtherUtils.givePropsValue("ali_oss_bucketName"), query.ossKey,new ByteArrayInputStream((query.content as String).getBytes("UTF-8")));
            ossClient.setObjectAcl(OtherUtils.givePropsValue("ali_oss_bucketName"), query.ossKey, CannedAccessControlList.PublicRead);
            ossClient.shutdown();
            //oss end
            return """{"status":"OK"}""";
        }
