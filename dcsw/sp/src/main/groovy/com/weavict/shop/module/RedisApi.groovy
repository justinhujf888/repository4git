...
//        orderBuyers
        //redis
//        orderBuyers在下面没有使用到
//        OrderBuyers orderBuyers =
        orderBean.joinTuan8Org(tuanInfo.id,order.id,payReturnOrder,order.tempMap["isCreate"],tuanInfo.tempMap["updateDatabase"]);
...
//                                redisUtil.expireAt("tuanBuyers_${tuanInfo.productId}_${order.orgration.id}",(new JDateTime(tuanInfo.endDate)).addDay(1).convertToDate());
        ...
        this.updateRedisbigTuanInfo(tuanInfo.id,order.id,payReturnOrder,product,order.tempMap["isCreate"],tuanInfo.tempMap["updateDatabase"]);
    }

    void updateRedisbigTuanInfo(tuanInfoId,orderId,PayReturnOrder payReturnOrder,Product product,boolean isCreaterOrder,boolean isCreateTuanInfo)
    {
        TuanInfo tuanInfo = orderBean.findObjectById(TuanInfo.class,tuanInfoId);
        Order order = orderBean.findObjectById(Order.class,orderId);
        tuanInfo?.cancelLazyEr();
        order?.cancelLazyEr();

        ObjectMapper objectMapper = new ObjectMapper();
        //"tuan_${payReturnOrder.productId}"
        this.buildRedisOrderTuan(objectMapper,tuanInfo);
        //"orgProductTuaning_${payReturnOrder.orgrationId}"
        if (isCreaterOrder)
        {
        ...
        //"tuanOrder_${tuanInfo.productId}_${payReturnOrder.orgrationId}"
        ...
        if (isCreaterOrder)
        {
            ...
            BuyerAppInfo buyerAppInfo = orderBean.findObjectById(BuyerAppInfo.class,new BuyerAppInfoPK(payReturnOrder.appId,payReturnOrder.buyerId));

            Twiter twiter = new Twiter();
            twiter.id = MathUtil.getPNewId();
            twiter.url = product.id;
            twiter.description = product.name + "； 我在社群发起了团购，欢迎大家跟进";
            ...
            twiterImages.path = product.masterImg;
            twiterImages.twiter = twiter;
...
//    processBigTuanPay方法已经改造，这是之前的版本
//    @Transactional
    void processBigTuanPay_bak(PayReturnOrder payReturnOrder)
    {
        ObjectMapper objectMapper = new ObjectMapper();
        OSSClient ossClient = OtherUtils.genOSSClient();
        def jsonSlpuer = new JsonSlurper();
...
//        orderBuyers在下面没有使用到
//        OrderBuyers orderBuyers =
        orderBean.joinTuan8Org(tuanInfo.id,order.id,payReturnOrder,order.tempMap["isCreate"]);
//        orderBuyers.cancelLazyEr();
//        orderBuyers.order.orgration = null;
//        orderBuyers.order.buyer = null;
//        orderBuyers.product.brand = null;
